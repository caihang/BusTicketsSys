...
void process_cli(int connfd, struct sockaddr_in client)
{
	...
	MYSQL_RES *result;
	MYSQL_ROW row;
    int i, rtn, offset, res = 0, num = 0, rowcount = 0;                   //i用于循环下标，num用于判断数据是否接收完毕,rtn是接受函数的返回值，offset是用于数据处理时的偏移量,res用于my_query返回值,rowcount用于获得行数
    ...
    person_data *now_person_data = NULL;//malloc(sizeof(person_data));                //用于存放接收到的人员信息中的每一个数据单元
	...
	res = mysql_query(&my_connection,"select * from bus_base_info");
    if(res != 0)                                                    //查数据库
    {
        lprintf(log, FATAL, "can not found the bus_base_info table\n");
        mysql_close(&my_connection);
        return;
    }
	else
	{
		result = mysql_store_result(&my_connection);    //获得结果集
		rowcount = mysql_num_rows(result);              //获得行数
		row = mysql_fetch_row(result);                  //获得结果的集
		if(mysql_num_rows(result)!=NULL)
		{
			do
			{
				if(!strcmp(now_data->terminal_ID,row[12]))
				{
					lprintf(log, INFO, "find the t_id\n");
		...
		{
			lprintf(log, INFO, "no data\n");
			mysql_close(&my_connection);
			return;
		}
	}

	res = mysql_query(&my_connection," select * from schedule");
	if(res != 0)                                                    //查数据库
	{
        lprintf(log, FATAL, "can not found the schedule table.\n");
        mysql_close(&my_connection);
        return;
    }
	else
	{
		...
		if(mysql_num_rows(result) != NULL)
		{
			do
			{
				if(!strcmp(now_data->schedule_num,row[1]))
				{
    ...
    /*数据无误，开始处理*/
	if(now_data->data_type == video_data_type)                     //如果数据类型是视频摘要信息
    {
    ...
    if(now_data->data_type == person_data_type)                                 //如果数据类型是人员信息
    {
